...
Pipeline order (v3 — Z-weighted filter + foot gap correction):
  ...
  4. Rigid bone length enforcement — FIRST pass (on mostly-valid data)
  5. One Euro + Butterworth jitter filtering (Z-weighted)
  6. Rigid bone length enforcement — SECOND pass (fix filter-induced drift)
...
def run_full_pipeline(
    ...
    3. NaN gap filling (linear interpolation)
    3.5. Foot gap correction (re-anchor heel/foot_index to ankle)
    4. Rigid bone length enforcement — FIRST pass
    5. One Euro + Butterworth jitter filtering (Z-weighted)
    6. Rigid bone length enforcement — SECOND pass (fix filter drift)
    6.5. Wrist consistency check (body/hand boundary)
    7. Floor plane alignment (optional)
    8. Foot contact detection & pinning
    ...
    """
    print("=" * 60)
    print("  FreeMoCap Enhanced Post-Processing Pipeline (v3)")
    print("=" * 60)
    ...
    t0 = time.time()
    print("\n[Stage 1/8] Outlier Detection & Initial Gap Fill")
    ...
    t0 = time.time()
    print("\n[Stage 2/8] Bone Length Outlier Detection")
    ...
    t0 = time.time()
    print("\n[Stage 3/8] Linear NaN Gap Filling")
    ...
    t0 = time.time()
    print("\n[Stage 3.5/8] Foot Gap Correction (ankle offset propagation)")
    ...
    if enforce_bones:
        t0 = time.time()
        print("\n[Stage 4/8] Rigid Bone Length Enforcement (1st pass — pre-filter)")
    ...
    t0 = time.time()
    print("\n[Stage 5/8] Combined Jitter Filter (OneEuro + Butterworth)")
    ...
    if enforce_bones:
        t0 = time.time()
        print("\n[Stage 6/8] Rigid Bone Length Enforcement (2nd pass — post-filter, 50% tol)")
    ...
    t0 = time.time()
    print("\n[Stage 7/8] Wrist Consistency Check")
    ...
    if align_floor:
        t0 = time.time()
        print("\n[Stage 7.5/8] Floor Plane Alignment")
    ...
    # Stage 6: Foot contact detection & pinning
    if fix_foot_sliding:
        t0 = time.time()
        print("\n[Stage 8/8] Foot Contact Detection & Sliding Fix")
    else:
        print("\n[Stage 8/8] Foot Sliding Fix: SKIPPED")
